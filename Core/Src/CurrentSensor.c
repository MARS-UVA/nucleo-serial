...
void writeRegisterCurrentSensor(CurrentSensor *cs, uint8_t registerAddress, uint16_t registerValue)
{
	...
	do {
		hal_status = HAL_I2C_Master_Transmit(cs->hi2c, cs->address << 1, data, 3, 100);

	    if (hal_status == HAL_ERROR)
		    writeDebugFormat("I2C write error (register address to read from): %d\r\n");
...
//	   hal_status = HAL_I2C_Master_Transmit(cs->hi2c, cs->address << 1, &registerAddress, 1, 100); // could be optimized for lower power consumption

	   if (hal_status == HAL_ERROR)
		   writeDebugFormat("I2C write error (register address to read from): %d\r\n");
   } while (hal_status != HAL_OK);

   cs->expectingRead = true;
//   HAL_I2C_Master_Receive(cs->hi2c, cs->address << 1, cs->rxBuff, 2, 100);
}
...
CurrentSensor CurrentSensorInit(I2C_HandleTypeDef *hi2c, uint16_t devAddress)
{
    ...
    if (HAL_I2C_IsDeviceReady(hi2c, devAddress << 1, 1, HAL_MAX_DELAY) != HAL_OK)
    {
      writeDebugFormat("WARNING: I2C device not found at address 0x%02X\r\n", devAddress);
    }
