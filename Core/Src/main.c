...
#define TCS34725_INTEGRATIONTIME_50MS   0x10 //0xEB  /* 50ms  - 20 cycles */ smaller values, longer time
#define TCS34725_GAIN_4X                0x02 //0x01  /* 4x gain  */
...
void getHSL(int *R, int *G, int *B, int *H, int *S, int *L)
{
	...
	float cMax = (rRed > rGreen) ? rRed : rGreen;
	float cMin = (rRed > rGreen) ? rGreen : rRed;
	cMax = (cMax > rBlue ) ? cMax : rBlue ;
	cMin = (cMin > rBlue ) ? rBlue :  cMin;
	...
	float luminosityDiff = (2 * luminosity - 1);

	if (delta != 0) {
		if (cMax == rRed) {
			hue = (60 * (rGreen - rBlue) / delta) + (rGreen < rBlue ? 6 : 0);
		} else if (cMax == rGreen) {
			hue = (60 * (rBlue - rRed) / delta) +2 ;
		} else if (cMax == rBlue) {
			hue = (60 * (rRed - rGreen) / delta) + 4;
		}
		luminosityDiff = (luminosityDiff > 0) ? luminosityDiff : -(luminosityDiff);
		saturation = delta / (1 - luminosityDiff);
	}
	*H = hue;
	*S = saturation * 100;
	*L = luminosity * 100;
...
int main(void)
{
  ...
  /* Reset of all peripherals, Initializes the Flash interface and the Systick. *1  HAL_Init();
